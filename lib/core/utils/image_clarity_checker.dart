...
class ImageClarityChecker {
  ...
  static Future<ImageClarityResult> checkFromBytes(Uint8List bytes) async {
    ...
    // Check if image has enough total pixels
    final totalPixels = image.width * image.height;
    if (totalPixels < minTotalPixels) {
      return ImageClarityResult.small(image.width, image.height);
    }

    // Also check that neither dimension is too tiny (prevents thumbnails)
    if (image.width < minAnyDimension || image.height < minAnyDimension) {
      return ImageClarityResult.small(image.width, image.height);
    }
